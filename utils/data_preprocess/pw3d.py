...
def pw3d_extract(dataset_path, out_path, debug=False):
    ...
    imagenames_seq = np.concatenate(imagenames_seq, axis=0)
    genders_seq = np.concatenate(genders_seq, axis=0)
    scales_seq = np.concatenate(scales_seq, axis=0)
    centers_seq = np.concatenate(centers_seq, axis=0)
    poses_seq = np.concatenate(poses_seq, axis=0)
    shapes_seq = np.concatenate(shapes_seq, axis=0)
    j3d_seq = np.concatenate(j3d_seq, axis=0)
    j2d_seq = np.concatenate(j2d_seq, axis=0)
    op_j2d_seq = np.concatenate(op_j2d_seq, axis=0)
    
    # store data
    if not os.path.isdir(out_path):
        os.makedirs(out_path)
    out_file = os.path.join(out_path,
        '3dpw_new_test_scale1.npz')
    np.savez(out_file, imgname=imagenames_seq,
                       gender=genders_seq,
                       scale=scales_seq,
                       center=centers_seq,
                       pose=poses_seq,
